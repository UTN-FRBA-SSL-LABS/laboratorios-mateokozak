...
> R: No, muestra 2050 como el total
...
> R: carrito_init inicializa un nuevo carrito, setea la cantidad de productos en él a 0
...
> R: Está en la línea 24, lo único que hace es sumar y asigna el precio del item a la variable total sin tener en cuenta la cantidad de items del mismo tipo

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Debería multiplicar el precio del item por la cantidad de veces que se agrega (c->items[i].precio * c->items[i].cantidad)
...
> R: Falla, el 5to carrito_agregar devuelve 1
...
> R: En la sentencia del if, <= debería ser < y la cantidad del carrito debería ser c->cantidad += p.cantidad para tener en consideración la cantidad de items del mismo tipo
...
> R: La función carrito_descuento nunca se ejecutó
...
> R: La diferencia es que un test unitario evalúa una parte del código mientras que el de integración evalúa varios componentes en conjunto. El test uniterio detectó el bug de carrito_total
...
> R: Es peligroso en C porque no chequea los límites de los arrays en tiempo de ejecución, mientras que en Python o Java el runtime impide el overflow, ya sea agrandando el array o devolviendo una excepción.
...
> R: Detecté que carrito agregar no contempla la cantidad de veces que se agrega un mismo item. Es decir que a pesar de ser máximo 4, yo puedo agregar un mismo artículo con cantidad 10. Este enfoque te hace pensar un poco mejor por qué y cómo se está haciendo el test
...
> R: Porque la función contemplaba únicamente el precio sin contemplar la cantidad de items del mismo tipo, entonces con cantidad 1 la cuenta da el valor correcto, pero con cantidad 2 sigue dando el mismo valor.
